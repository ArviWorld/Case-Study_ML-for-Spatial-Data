...
explainer_rf <- DALEX::explain(model_rf, 
                               ...
                               y = testDat[, response], 
                               label = "Random Forest",
                               predict_function = function(m, d) predict(m, d))
...
# Residual plots
residual_plots[[1]] + residual_plots[[2]] + residual_plots[[3]]
...
```{r stacking sentinel, echo=FALSE, message=FALSE}
...
rf_df <- as.data.frame(predictions_rf_all, xy = TRUE, na.rm = TRUE)
xgb_df <- as.data.frame(predictions_xgb_all, xy = TRUE, na.rm = TRUE)
svm_df <- as.data.frame(predictions_svm_all, xy = TRUE, na.rm = TRUE)

# Rename the prediction column for clarity (assuming 1 layer per raster)
names(rf_df)[3] <- "class"
names(xgb_df)[3] <- "class"
names(svm_df)[3] <- "class"
...
p_rf <- ggplot(rf_df) + 
  geom_raster(aes(x,y, fill = as.factor(class))) +
...
# Plot for Support Vector Model (SVM)
p_svm <- ggplot(svm_df) +
  geom_raster(aes(x,y, fill = as.factor(class))) +
  scale_fill_viridis_d(name="Class") +
  coord_equal() +
  ggtitle('Support Vector Model (SVM)') +
  theme_minimal()

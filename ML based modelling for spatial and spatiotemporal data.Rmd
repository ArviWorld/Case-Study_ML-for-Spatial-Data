...
library(patchwork)    # Combines multiple ggplots into a single layout
...
**Splitting data into training and test data** In order to keep data for a later (nearly) independent validation as well as to limit the number of data points so that the model training won't take long time, we split the total data set into 30% training data and 70% test data. The more testing (unknown) data the higher the model accuracy. Caret's `createDataPartition` takes care that the class distribution is the same in both datasets.
...
# Creating class weights to adjust the imbalance
# Create class weights (inverse of frequency)
class_weights <- 1 / table(trainDat$Class)
class_weights <- class_weights / sum(class_weights)

kable(class_weights, caption = "Class weights") %>%
  kable_styling(full_width = F, position = "left")
...
- very high classification (*accuracy = 0.9934*), 
- good overall class performance (*balanced accuracy = 0.9828*) and
- a balanced precision and recall (*mean F1 score = 0.969*).
...
- `Water`, `Grassland`, `Gorse`, `Forestry`, `Bush` are 100% predicted and classfied correctly.
- Out of 182 `Urban` Class, 8 were predicted as `Sand` and 10 as `Bare`.
- Out of 221 `Sand` Class, 12 were predicted as `Urban`.
- Out of 455 `Bare` class, 5 were predicted as `Urban`.
...
model_xgb <- train(x = trainDat[, predictors],
                   ...
                   method = "xgbTree",
                   trControl = control,
                   weights = class_weights[trainDat$Class],
                   tuneGrid = xgb_grid)
...
```
The variable importance scores for each predictor variable is derived. The variable importance plot shows which features contribute most to the classification task, with the `B03`, `Yellowness` & `B05` being among the top 3 important predictors for the XGBoost model.
...
- the least uncertainity (*logLoss = 0.0393*), 
- nearly perfect classification (*AUC score = 0.999*),
- perfect aggrement beyond chance (*Kappa = 0.987*),
- very high classification (*Accuracy = 0.992*),
- good overall class performance (*balanced accuracy = 0.983*), and 
- a balanced precision and recall (*mean F1 score = 0.969*).
These score are very similar to the Random Forest Model and will be compared in the results and discussion section.

**Predicting the test data with confusion matrix and scoring metrics**
To evaluate the performance of the XGBoost model on the test dataset, we can create a confusion matrix and calculate scoring metrics such as accuracy, sensitivity, and specificity. The confusion matrix provides insights into how well the model performs on the test dataset.
...
- Out of 182 `Urban` Class, 8 were predicted as `Sand`, 2 as `Gorse` and 11 as `Bare`
- Out of 221 `Sand` Class, 17 were predicted as `Urban` and 2 as `Gorse`.
- Out of 634 `Gorse` Class, 6 were predicted as `Grassland`
- Out of 733 `Bush` Class, 1 was predicted as `Water`
- Out of 455 `Bare` class, 8 were predicted as `Urban` and 1 as `Bush`.

This means that the `Urban`, `Bare`, `Bush`, `Gorse` and `Sand` are very little false positives with the XGBoost model. 
...
svm_grid <- expand.grid(
  C = 2^(-2:2),         # Cost parameter
  sigma = 2^(-2:2))     # Kernel parameter
...
**Predicting the test data with confusion matrix and scoring metrics** 
To evaluate the performance of the SVM model on the test dataset, we can create a confusion matrix and calculate scoring metrics such as accuracy, sensitivity, and specificity. The confusion matrix provides insights into how well the model performs on the test dataset.

```{r svm_confusion_matrix, echo=TRUE, warning=FALSE, message=FALSE}

# Predict on the test dataset
predictions_svm <- predict(model_svm, newdata = testDat[, predictors])
probs_svm <- predict(model_svm, newdata = testDat[, predictors],
                           probability = TRUE)
...
plotRGB(sentinel, stretch = "lin", main = "Sentinel RGB") 

plot(raster_rf, col = palette,
     axes = FALSE, main = "RF predictions", legend = TRUE)

plot(raster_xgb, col = palette,
     axes = FALSE, main = "XGB predictions", legend = TRUE)
...
# Combine all
all_measures <- bind_rows(rf_measures, xgb_measures, svm_measures)
...
**Feature reduction - Retraining the model with the only the top variables**
As seen in the variable importance plot, `B05`,`NDVI` & `Yellowness` contributes most to the Random Forest model. Hence, we chose the top 3 important predictors for retraining the model and then apply this in the test data set. 

``` {r feature training, echo=TRUE, warning=FALSE, message=FALSE}

# selecting the top 3 variable predictors 
predictors_rf <- c("B05", "NDVI", "yellowness")

# Train Random Forest model
set.seed(100)
model_p_rf <- train(trainDat[, predictors_rf],
                    trainDat[, response],
                    method = "rf",
                    trControl = control
                    class.weights = class_weights,
...
# Print model summary
kable(model_p_rf$results, caption = "Random Forest Model Results") %>%
  kable_styling(full_width = F, position = "left")

```
The Random Forest model has been retrained successfully with the selected variables. The model achieved excellent performance across two values (2 & 3) of `mtry`, with the least logLoss results at 2. At `mtry` at 2, the model shows the least uncertainity (logLoss = 0.0238), nearly perfect classification (AUC score = 0.999), good overall class performance (balanced accuracy = 0.9797) and a balanced precision and recall (mean F1 score = 0.962). 

...
**1. DALEX Uncertainity Analysis** To gain insights into the uncertainty of the model predictions, we can use the DALEX (Descriptive mAchine Learning EXplanations) package designed to help understand and explain complex machine learning models. DALEX allows to explore the model performance and diagnostics for each model. This allows us to analyze the model's predictions and understand how different features contribute to the uncertainty in the predictions.
...
par(mfrow = c(1,2))
plot(mp_rf, main = 'Random Forest Performance')
plot(mp_xgb, main = 'XGBoost Performance')
par(mfrow = c(1,1))

# DALEX - Predict Diagnostics
# Compute diagnostocs for each model
diag_rf <- predict_diagnostics(explainer_rf, N=NULL)
diag_xgb <- predict_diagnostics(explainer_xgb, N=NULL)

# Plotting uncertainty histograms
par(mfrow = c(1,2))
hist(diag_rf$prediction_entropy, breaks = 30, col = "skyblue", 
     main = "RF Prediction Entropy", xlab = "Entropy")
hist(diag_xgb$prediction_entropy, breaks = 30, col = "salmon", 
     main = "XGB Prediction Entropy", xlab = "Entropy")
par(mfrow = c(1,1))
...
entropy_raster_rf <- predictors_stack
entropy_raster_xgb <- predictors_stack

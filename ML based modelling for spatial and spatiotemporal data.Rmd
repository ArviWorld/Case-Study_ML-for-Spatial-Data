...
raster_rf  <- setValues(raster(predictors_stack), predictions_rf_all)
raster_xgb <- setValues(raster(predictors_stack), predictions_xbg_all)

# Define class names and palette
classes <- unique(training$Class)  # Replace with your actual column name
n_classes <- length(classes)
palette <- brewer.pal(n_classes, "Set2")
...
plotRGB(sentinel, r = 1, g = 2, b = 3, stretch = "lin", main = "RF on Sentinel RGB")
plot(raster_rf, col = palette, alpha = 0.5, 
     add = TRUE, axes = FALSE, legend = TRUE)

plotRGB(sentinel, r = 1, g = 2, b = 3, stretch = "lin", main = "XGB on Sentinel RGB")
plot(raster_xgb, col = palette, alpha = 0.5, 
     add = TRUE, axes = FALSE, legend = TRUE)

# Add legend
legend("topleft", legend = classes, col = palette, 

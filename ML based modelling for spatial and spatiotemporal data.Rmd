...
# Balance the class
balanced_train <- upSample(x = trainDat[, predictors], y = trainDat$Class)
balanced_test <- upSample(x = testDat[, predictors], y = testDat$Class)

bal_training_class = round(prop.table(table(balanced_train$Class)) * 100, 2)
bal_training_class = round(prop.table(table(balanced_test$Class)) * 100, 2)

kable(bal_training_class, caption = "Training class balance") %>%
  kable_styling(full_width = F, position = "left")
kable(bal_training_class, caption = "Test class balance") %>%
  kable_styling(full_width = F, position = "left")

balanced_train$Class <- balanced_train$.Class
balanced_train$.Class <- NULL

balanced_test$Class <- balanced_test$.Class
balanced_test$.Class <- NULL
...
# Machine Learning Model {#machine-learning-model}
The split data is now ready for training machine learning models. We will implement three different algorithms: Random Forest and XGBoost. Each model will be trained on the training dataset and evaluated on the test dataset.
...
probs_rf <- predict(model_rf, newdata = testDat[, predictors],
                   type = 'prob')
...
probs_xgb <- predict(model_rf, newdata = testDat[, predictors],
                   type = 'prob')
...
scores_rf = score(explainer_rf)
scores_xgb = score(explainer_xgb)
...
scores_df <- rbind(scores_rf, scores_xgb)

# Reorder columns (optional)
scores_df <- scores_df[, c("model", setdiff(names(scores_df), "model"))]

# View results
print(scores_df)

```
**2. Probability-based Entropy Maps**

```{r entropy analysis}

# Entropy function per row (Shannon Entropy)
entropy_row <- function(probs) {
  -sum(probs * log(probs + 1e-10))  # avoid log(0)
}

# Compute entropy
entropy_rf <- apply(probs_rf, 1, entropy_row)
entropy_xgb <- apply(probs_xgb, 1, entropy_row)

```

``` {r entropy maps}

testDat$entropy_rf  <- entropy_rf
testDat$entropy_xgb <- entropy_xgb

# Plot Entropy Map (e.g., for SVM)
ggplot(testDat, aes(x = lon, y = lat, fill = entropy_rf)) +
  geom_raster() +
  scale_fill_viridis_c() +
  labs(title = "Entropy Map - RV", fill = "Entropy")

ggplot(testDat, aes(x = lon, y = lat, fill = entropy_xgb)) +
  geom_raster() +
  scale_fill_viridis_c() +
  labs(title = "Entropy Map - XGB", fill = "Entropy")

```

**3. Model Disagreement Map**

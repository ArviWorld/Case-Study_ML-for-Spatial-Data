...
predict_raster <- function(model, raster) {
  raster_df <- as.data.frame(raster, na.rm=TRUE)
  pred <- predict(model, raster_df)
  out <- raster[[1]]
  out[] <- NA
  out[!is.na(raster[[1]])] <- as.numeric(pred)
  return(out)
}

# Predict maps
map_rf <- predict_raster(model_rf, sentinel)
map_xgb <- predict_raster(model_xgb, sentinel)
map_svm <- predict_raster(model_svm, sentinel)

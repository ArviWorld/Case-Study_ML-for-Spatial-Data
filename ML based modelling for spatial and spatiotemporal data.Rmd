...
mapview(raster_rf, col.regions = palette, legend = TRUE, na.color = NA) + 
  mapview(training, zcol = "Class", col.regions = "black", alpha = 0.5)

mapview(raster_xgb, col.regions = palette, legend = TRUE, na.color = NA) + 
  mapview(training, zcol = "Class", col.regions = "black", alpha = 0.5)
...
**Feature reduction - Retraining the model with the only the top variables** As seen in the variable importance plot, `B05`,`NDVI` & `Yellowness` contributes most to the Random Forest model. Hence, we chose the top 3 important predictors for retraining the model and then apply this in the test data set.

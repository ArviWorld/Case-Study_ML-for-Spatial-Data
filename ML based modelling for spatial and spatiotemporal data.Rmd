...
The Random Forest model has been trained successfully, and we can see the variable importance scores for each predictor variable. The variable importance plot shows which features contribute most to the classification task, with the `B05`,`NDVI` & `Yellowness` being among the most important predictors.

**Predicting the test data with confusion matrix and scoring metrics** To evaluate the performance of the Random Forest model on the test dataset, we can create a confusion matrix and calculate scoring metrics such as accuracy, sensitivity, and specificity. The confusion matrix provides insights into how well the model performs on the test dataset.
...
predictions_svm <- predict(model_svm, newdata = pixels_df, type = "raw")
...
raster_svm <- setValues(raster(predictors_stack), predictions_svm)
...
raster_svm <- rast(raster_svm); levels(raster_svm) <- data.frame(value = 1:8, class = classes)
...
plot(raster_svm, col = palette,
     axes = FALSE, main = "SVM predictions", legend = TRUE)
...
svm_measures <- measures_to_df(mp_svm$measures, "SVM")

...
**Confusion matrix and Scoring metrics on the test data** 
...
confusion_rf <- confusionMatrix(predictions_rf, actual_rf)
...
# Define hyperparameter tuning grid (optional but recommended)
...
if (is.null(model_xgb$finalModel)) stop("XGBoost model training failed.")
if (is.null(model_xgb$results)) stop("No results returned.")
...
confusion_xgb <- confusionMatrix(predictions_xgb, actual_xgb)
...
```
```{r svm_summary, echo=FALSE, message=FALSE}
...
# Construct the confusion matrix
confusion_svm <- confusionMatrix(predictions_svm, actual_svm)
...
plot_variable_importance <- function(explainer) {
  ...
  ggplot(variable_importance, aes(x = reorder(variable, importance), y = importance)) +
    geom_bar(stat = "identity", fill = "steelblue") +
    coord_flip() +
    labs(title = paste("Variable Importance -", explainer$label),
         x = "Variable", y = "Importance Score") +
    theme_minimal()
}

# Create plots for variable importance for each model
plots <- lapply(explainers, plot_variable_importance)

# Plot variable importance for each model
grid.arrange(grobs = plots, ncol = 1)

# Plot model predictions
plot_model_predictions <- function(explainer) {
  model_predictions <- model_performance(explainer)
  ggplot(model_predictions, aes(x = observed, y = predicted)) +
    geom_point() +
    geom_abline(slope = 1, intercept = 0, color = "red") +
    labs(title = paste("Model Predictions -", explainer$label),
         x = "Observed", y = "Predicted") +
    theme_minimal()
}

plots_predictions <- lapply(explainers, plot_model_predictions)

# Plot model predictions for each model
grid.arrange(grobs = plots_predictions, ncol = 1)
...
```{r spatial_predictions, echo=FALSE, message=FALSE}

# Create a function to plot spatial predictions for each model
plot_spatial_predictions <- function(predictions, title) {
  # Convert predictions to a raster
  pred_raster <- raster(predictions)
  
  # Plot the raster with a color palette
  plot(pred_raster, main = title, col = terrain.colors(10), axes = FALSE, box = FALSE)
}

# Plot spatial predictions for each model
plot_spatial_predictions(predictions_rf, "Random Forest Predictions")
plot_spatial_predictions(predictions_xgb, "XGBoost Predictions")
plot_spatial_predictions(predictions_svm, "Support Vector Machine Predictions")

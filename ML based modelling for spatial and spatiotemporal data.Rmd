...
viewRGB(sentinel, r = 3, g = 2, b = 1, scale = 65535, stretch = "lin", main = "Sentinel RGB Image") + mapview(training)
...
kable(extr, caption = "Merged Data") %>%
...
The Random Forest model has been trained successfully, and we can see the variable importance scores for each predictor variable. The variable importance plot shows which features contribute most to the classification task, with the  being among the most important predictors.

**Predicting the test data and using confusion matrix and scoring metrics**
...
explainer_rf <- DALEX::explain(model_rf, data = trainDat[, predictors], y = trainDat[, response], label = "Random Forest")
explainer_xgb <- DALEX::explain(model_xgb, data = trainDat[, predictors], y = trainDat[, response], label = "XGBoost")
explainer_svm <- DALEX::explain(model_svm, data = trainDat[, predictors], y = trainDat[, response], label = "Support Vector Machine")

# Create a list of explainers
explainers <- list(explainer_rf, explainer_xgb, explainer_svm)

# Plot model residuals
plot_model_residuals <- function(explainer) {
  perf <- DALEX::model_performance(explainer)
  plot(perf) + ggtitle(paste("Residual Plot -", explainer$label))
}

# Generate residual plots 
residual_plots <- lapply(explainers, plot_model_residuals)

# Residual plots
for (p in residual_plots) print(p)
...
```{r staacking sentinel, echo=FALSE, message=FALSE}
...
all_pixels_df <- as.data.frame(predictors_stack, na.rm = FALSE)

# Predict for all raster cells for each model
predictions_rf_all <- predict(model_rf, newdata = all_pixels_df)
predictions_xbg_all <- predict(model_xgb, newdata = all_pixels_df)
predictions_svm_all <- predict(model_svm, newdata = all_pixels_df)

```

``` {r predicted map vis}

# Get unique classes and assign consistent colors
all_classes <- sort(unique(trainDat[[response]]))
class_colors <- brewer.pal(length(all_classes))  # You can customize this
names(class_colors) <- all_classes
my_labels <- paste("Class", all_classes)

# Use reference raster to get shape
ref_raster <- raster(sentinel)

# Assign global consistent colors
  all_classes <- sort(unique(trainDat[[response]]))
  class_colors <- rainbow(length(all_classes))
  names(class_colors) <- all_classes
  my_labels <- paste("Class", all_classes)

# Create a function to plot spatial predictions for each model
plot_spatial_predictions <- function(predictions, 
                                     reference_raster, 
                                     title,
                                     legend_labels = NULL, 
                                     legend_position = "top") 
  {
  # Create a raster template with same dimensions and extent as reference (e.g. sentinel)
  pred_raster <- raster(ref_raster[[5]])
  # Safety check: make sure prediction length matches raster cells
  if (length(predictions) != ncell(pred_raster[[5]])) {
    stop("Length of predictions does not match number of raster cells.")
  }
  
  values(pred_raster) <- as.numeric(predictions)

  # Plot RGB base
  plotRGB(reference_raster, r = 3, g = 2, b = 1, 
          scale = 65535, stretch = "lin", 
          main = paste("Overlay:", title))

  # Mask undefined classes
  pred_raster[!predictions %in% all_classes] <- NA

  # Add transparent overlay of predictions
  plot(pred_raster, 
       col = class_colors[as.character(all_classes)],
       legend = FALSE, 
       add = TRUE, 
       alpha = 0.4)
...
# Plot spatial predictions for each model
par(mfrow = c(1, 3))  # 3 plots in one row

plot_spatial_predictions(predictions_rf_all, sentinel, "Random Forest",
                         my_labels)
plot_spatial_predictions(predictions_xbg_all, sentinel, "XGBoost",
                         my_labels)
plot_spatial_predictions(predictions_svm_all, sentinel, "SVM",
                         my_labels)
